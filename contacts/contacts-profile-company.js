...
// Inline helper mirroring the main Contacts people Links rendering
function renderPersonLinksInline(person) {
  const decorateIcon = (svg) => svg.replace('<svg', '<svg class="w-4 h-4 text-gray-600 dark:text-gray-300"');
  const getField = (key) => {
    if (person[key]) return person[key];
    if (person.socials && person.socials[key]) return person.socials[key];
    return null;
  };
  const iconMap = {
  ...
  const socialKeys = [
    { key: 'linkedin', value: getField('linkedin') },
    { key: 'facebook', value: getField('facebook') },
    { key: 'x', value: getField('x') || getField('twitter') },
    { key: 'instagram', value: getField('instagram') },
    { key: 'tiktok', value: getField('tiktok') },
    { key: 'whatsapp', value: getField('whatsapp') },
    { key: 'youtube', value: getField('youtube') },
    { key: 'snapchat', value: getField('snapchat') },
    { key: 'threads', value: getField('threads') },
    { key: 'reddit', value: getField('reddit') },
    { key: 'pinterest', value: getField('pinterest') },
    { key: 'globe', value: getField('website') },
  ...
  socialKeys.forEach(({ key, value }) => {
    links.push(
      `<a href="${value}" target="_blank" rel="noreferrer" class="contact-link-icon inline-flex items-center justify-center w-8 h-8 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 mx-0.5" aria-label="${key}">
