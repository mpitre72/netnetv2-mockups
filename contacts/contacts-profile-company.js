...
export function renderCompanyProfile(data, profileState, mockReportData) {
  ...
  const socialEntries = [
    ...
    { key: 'facebook', value: data.socials?.facebook },
    { key: 'x', value: data.socials?.twitter },
    { key: 'instagram', value: data.socials?.instagram },
    { key: 'tiktok', value: data.socials?.tiktok },
    { key: 'whatsapp', value: data.whatsapp },
    { key: 'youtube', value: data.youtube },
    { key: 'snapchat', value: data.snapchat },
    { key: 'threads', value: data.threads },
    { key: 'reddit', value: data.reddit },
    { key: 'pinterest', value: data.pinterest },
    { key: 'globe', value: data.website ? `https://${data.website}` : null },
  ].filter(s => s.value);
  ...
  const renderSocialRows = () => {
    ...
    return socialEntries.map(s => `
      <div class="flex items-center gap-4">
        ...
        <div class="flex-1">
          <a href="${s.value}" target="_blank" rel="noreferrer" class="text-netnet-purple hover:underline break-all">${s.value}</a>
        </div>
      </div>
    `).join('');
  };
  ...
  const renderPeopleRows = () => {
    if (!people.length) {
      return `<div class="text-sm text-gray-500 dark:text-gray-400">No people have been added for this company yet.</div>`;
    }
    return people.map(person => {
      const location = [person.city, person.state].filter(Boolean).join(', ') || '-';
      const mobile = person.mobile || person.phone || '-';
      const emailLink = person.email ? `<a href="mailto:${person.email}" class="contact-link-icon inline-flex items-center justify-center w-8 h-8 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 mx-0.5" aria-label="Email ${person.name}"><svg class="w-4 h-4" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2Z"/><path d="m22 6-10 7L2 6"/></svg></a>` : '';
      const linkedIn = person.linkedin ? `<a href="${person.linkedin}" target="_blank" rel="noreferrer" class="contact-link-icon inline-flex items-center justify-center w-8 h-8 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 mx-0.5" aria-label="LinkedIn"><svg class="w-4 h-4" viewBox="0 0 24 24" aria-hidden="true" fill="none" stroke="currentColor" stroke-width="2"><path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2h0a2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6Z" /><rect width="4" height="12" x="2" y="9" /><circle cx="4" cy="4" r="2" /></svg></a>` : '';
      return `
        <div class="contacts-row company-people-row flex flex-col md:grid md:grid-cols-[1.2fr,1fr,0.8fr,0.9fr,1fr] md:items-center gap-2 md:gap-3 py-2 border-b border-gray-200 dark:border-gray-800 last:border-0">
          ...
          <div class="bg-white dark:bg-gray-800 rounded-xl shadow border border-gray-200 dark:border-gray-700 p-6 space-y-8">
            <div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                <div class="md:col-span-2"><label class="${labelClass}">Company Name</label><div class="${viewValueClass} text-lg">${data.name}</div></div>
                <div><label class="${labelClass}">Industry</label><div class="${viewValueClass}">${data.industry || '-'}</div></div>
                <div><label class="${labelClass}">Website</label><a href="https://${data.website}" target="_blank" class="text-netnet-purple hover:underline break-all">${data.website}</a></div>
                <div class="md:col-span-2"><label class="${labelClass}">Description</label><p class="text-sm text-gray-600 dark:text-gray-300 leading-relaxed">${data.description || 'No description.'}</p></div>
              </div>
            </div>
            <div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div><label class="${labelClass}">Office Phone</label><div class="${viewValueClass}">${data.phone || '-'}</div></div>
            ...
            <div>
              <h4 class="text-sm font-bold text-gray-900 dark:text-white border-b border-gray-200 dark:border-gray-700 pb-1 mb-3">People at this Company</h4>
              <div class="hidden md:grid md:grid-cols-[1.2fr,1fr,0.8fr,0.9fr,1fr] text-xs uppercase text-gray-500 dark:text-gray-400 pb-2">
                <div>Name</div>
                <div>Title</div>
                <div>Links</div>
                <div>Mobile</div>
