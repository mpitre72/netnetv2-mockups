...
function renderEmptyState(title, desc, btnText) {
  return `
    <section class="relative mx-auto max-w-2xl">
      <div class="mx-auto w-full rounded-2xl border bg-white/90 dark:bg-slate-800/90 backdrop-blur-sm shadow-xl border-black/10 dark:border-white/10 px-6 py-8 md:px-8 md:py-10 text-center">
        ...
        <h2 class="text-xl md:text-2xl font-semibold text-slate-900 dark:text-white">${title}</h2>
        <p class="mt-2 text-sm md:text-base text-slate-600 dark:text-slate-300">${desc}</p>
        <div class="mt-6 flex items-center justify-center gap-3">
          ...
          <button type="button" class="inline-flex h-10 items-center justify-center rounded-md px-4 text-sm font-semibold text-white transition-colors" style="background:#711FFF;" onmouseover="this.style.background='#6020df';" onmouseout="this.style.background='#711FFF';">${btnText}</button>
        </div>
      </div>
    </section>
  `;
...
export function renderMePage(page, container = document.getElementById('app-main')) {
  ...
  let html = '';
  if (page === 'time') {
    html = renderEmptyState("You Don't Have Any Time Logged yet", "Once you start logging time, it will appear here in your personal timesheets.", '+ New Time Entry');
  } else if (page === 'performance') {
    html = renderEmptyState("You Don't Have Any performance KPI's Yet", "As you complete work and log time, your personal performance KPI’s will appear here.", '+ View Performance Tips');
  } else {
    html = renderEmptyState('You do not have any tasks assigned', 'Once tasks are assigned, they will appear here', '+ New Task');
  }
